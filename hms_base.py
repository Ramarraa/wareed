...
class product_template(models.Model):
    ...
    check_consulations=fields.Boolean("Consulation Services")
...
class ACSConsumableLine(models.Model):
    ...
    # def action_register_payment_cons(self):
    #     action = self.env.ref('account.action_account_payments').read()[0]
    #     # action['domain'] = [('appointment_id', '=', self.id), ('parent.is_lab', '=', True)]
    #     action['views'] = [(self.env.ref('account.view_account_payment_form').id, 'form')]
    #     action['context'] = {
    #         'default_partner_id': self.appointment_id.patient_id.partner_id.id,
    #         'default_amount': self.appointment_id.total,
